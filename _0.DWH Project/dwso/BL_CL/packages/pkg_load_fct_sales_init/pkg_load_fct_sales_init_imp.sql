   ...
   CREATE OR REPLACE PACKAGE BODY pkg_load_fct_sales_init IS
   ----------------------------------------------------------------------
    -- Load CE_SALES 
    ----------------------------------------------------------------------
...
FOR counter IN 0..count_of_loops LOOP
    ...
    and p2.start_dt<=date_id and p2.end_dt >c.date_id
...
--------------------------------------------------------------------------------
            ROLLBACK;
            RAISE;
END prc_load_fct_sales_init;
  
END pkg_load_fct_sales_init;
