...
INSERT INTO CE_CHANNELS(
            ...
            TA_INSERT_DT )
        SELECT 
                   SEQ_CE_CHANNEL_ID.NEXTVAL,cr. CHANNEL_ID,cr. CHANNEL_SOURCE_SYSTEM,cr.CHANNEL_SOURCE_ENTITY,cr.CHANNEL_DESCRIPTION,cr. CHANNEL_CLASS_ID,SYSDATE, SYSDATE 
                   FROM (
                   Select Distinct
                   C.CHANNEL_ID                           ,
              ...
              FROM
                  ...
                  LEFT JOIN CE_CHANNEL_CLASSES C2 ON C.channel_class_id = C2.CHANNEL_CLASS_ID) cr;
...
--------------------------------------------------------------------------------
        ...
        WHERE
            sa_table_name = 'SRC_PROMOTIONS';
     
        -- WORK WITH EXISTING PROMO
     --  select * from bl_cl.WORK_PROMOTIONS_SCD
        MERGE INTO bl_cl.WORK_PROMOTIONS_SCD CH
        USING (                        
Select t.*, p.promotion_category_id
from
(SELECT SA_SOURCE_SYSTEM_1.SRC_PROMOTIONS.promo_id as mergeKey, SA_SOURCE_SYSTEM_1.SRC_PROMOTIONS.*
  FROM SA_SOURCE_SYSTEM_1.SRC_PROMOTIONS 
  )T
left join ce_promotion_categories p
on t.promo_category_id=p.PROMOTION_CATEGORY_SRCID
)t
         ON ( CH.PROMOTION_SRCID = T.mergeKey )
        WHEN MATCHED  
        THEN UPDATE
        SET 
        end_dt=TO_DATE(SUBSTR (to_char(t.promo_begin_date),1,10), 'YYYY-MM-DD'),
        is_active='inactive',
        TA_UPDATE_DT = SYSDATE
        WHERE 
                ch.is_active = 'active'
                AND TO_DATE(SUBSTR (to_char(t.promo_begin_date),1,10), 'YYYY-MM-DD') >CH.start_dt
               AND t.TA_UPDATE_DT > p_last_update_dt; 
...
(SELECT SA_SOURCE_SYSTEM_1.SRC_PROMOTIONS.promo_id as mergeKey, SA_SOURCE_SYSTEM_1.SRC_PROMOTIONS.*
  FROM SA_SOURCE_SYSTEM_1.SRC_PROMOTIONS
  Where TA_UPDATE_DT > p_last_update_dt
  )T
left join ce_promotion_categories p
on t.promo_category_id=p.PROMOTION_CATEGORY_SRCID)t
         ON ( CH.PROMOTION_SRCID = T.mergeKey 
         and TO_DATE(SUBSTR (to_char(t.promo_begin_date),1,10), 'YYYY-MM-DD')=start_dt)
        WHEN not  MATCHED 
        THEN insert 
        (
    PROMOTION_ID,
 ...
 VALUES
            ...
            TO_DATE(SUBSTR (to_char(t.promo_begin_date),1,10), 'YYYY-MM-DD') ,
...
---------Explicit Cursor usage--------------------------------------------------
        ...
        CLOSE CH_CL;
         ...
         EXECUTE IMMEDIATE 'truncate table bl_3nf.ce_promotions_scd';
    ...
    SYSDATE
        ...
        Insert into  CE_Promotions_scd(PROMOTION_ID,
    PROMOTION_SRCID,
    PROMOTION_SOURCE_SYSTEM,
    PROMOTION_SOURCE_ENTITY,
    PROMOTION_NAME,
    ...
    TA_UPDATE_DT,
    TA_INSERT_DT)
Select tt.promotion_id,t.promotion_srcid, t.promotion_source_system, t.promotion_source_entity, t.promotion_name, t.promotion_category_id, t.start_dt, t.end_dt, t.is_active, t.cost,t.ta_update_dt, t.ta_insert_dt
from 
BL_CL.WORK_PROMOTIONS_SCD t
inner join (
Select min (promotion_id) as promotion_id, promotion_srcid
from BL_CL.WORK_PROMOTIONS_SCD t
Group by promotion_srcid ) tt
on t.promotion_srcid=tt.promotion_srcid;
COMMIT; 
--------------------------------------------------------------------------------
---------Logging on finish procedure--------------------------------------------
...
--------------------------------------------------------------------------------       
        ...
        WHERE sa_table_name = 'SRC_PROMOTIONS';

     --   log_msg := 'PRM_MTA_INCREMENTAL_LOAD was updated';
      --  BL_CL.write_log(log_user, log_msg, sysdate);
...
MERGE INTO ce_countries  ch
    ...
    FROM  SA_SOURCE_SYSTEM_1.SRC_COUNTRIES) t
