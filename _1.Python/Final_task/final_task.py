""" An inverted index is a dictionary, where keys are words
and values are indexes of documents where these keys have occurred"""
...
class EncodedFileType(FileType):
    ...
    def __call__(self, string):
        ...
        # other arguments are file names
        try:
...
class InvertedIndex:
    """
    This module is needed to extract the inverted index from the document.
    """
    def __init__(self, words_ids: Dict[str, List[int]]):
        self.words_ids = words_ids

    def query(self, words: List[str]) -> List[int]:
        """Returns a list of related documents for the given query"""
        indexes = self.words_ids.get(words[0])

        if indexes is None:     # If the word does not exist in any document
            return []
        else:
            indexes = set(indexes)

        for word in words:
            indexes &= set(self.words_ids.get(word))
    ...
    def dump(self, filepath: str) -> None:
        """
        Allow reverse index documents to be written to temporary directory or local storage
        :param filepath: path to file with documents
        :return: None
        """
        with open(filepath, "w") as index_file:
            json.dump(self.words_ids, index_file)
...
def load_documents(filepath: str) -> Dict[int, str]:
    ...
    :return: Dict[int, str]
    """
    with open(filepath, encoding='utf-8') as documents:

        docs_ids = []
        contents = []

        for line in documents:
            docs_ids, content = line.lower().split('\t', 1)
            docs_ids.append(int(docs_ids))
            contents.append(content)

        return dict(zip(docs_ids, contents))
...
def build_inverted_index(documents: Dict[int, str]) -> InvertedIndex:
    ...
    """
    with open(PATH_TO_STOP_WORDS, encoding='utf-8') as stop_words:

        stop_words = set(stop_words.read())
        index_dict = dd(list)   # use default dict to build our index

        for docs_ids, doc_text in documents.items():
            all_words = set(simple_preprocess(doc_text, deacc=True))
            #all_words = set(re.split(r"\W+", doc_text))
            valid_words = all_words - stop_words
            for word in valid_words:
                index_dict[word].append(docs_ids)
