CREATE OR REPLACE PACKAGE BODY PKG_LOAD_SRC_2_TABLES IS
    ----------------------------------------------------------------------
    -- Load SRC_CHANNELS 
    ----------------------------------------------------------------------
    PROCEDURE   PRC_LOAD_SRC_2_CHANNELS IS

        COUNT_OF_ROWS_BEFORE NUMBER;
        COUNT_OF_ROWS_AFTER  NUMBER;
        L_ROWS               NUMBER;
        ROW_CH_CL            SA_SOURCE_SYSTEM_2.SRC_CHANNELS%ROWTYPE;
        L_PROCEDURE_NAME     VARCHAR2(100) := 'prc_load_SRC_2_CHANNELS';
        ...
        SELECT
            *
        FROM
            SA_SOURCE_SYSTEM_2.SRC_CHANNELS;
...
---------Merhe logic------------------------------------------------------------
        ...
        MERGE INTO SA_SOURCE_SYSTEM_2.SRC_CHANNELS CH
        USING (
                    SELECT DISTINCT 
                    channel_id  ,
                    channel_desc ,
...
--------------------------------------------------------------------------------
            ROLLBACK;
            RAISE;
    END   PRC_LOAD_SRC_2_CHANNELS;
    
   ----------------------------------------------------------------------
    -- Load SRC_COUNTRIES 
    ----------------------------------------------------------------------
    PROCEDURE   PRC_LOAD_SRC_2_Countries IS

        COUNT_OF_ROWS_BEFORE NUMBER;
        COUNT_OF_ROWS_AFTER  NUMBER;
        L_ROWS               NUMBER;
        ROW_CH_CL            SA_SOURCE_SYSTEM_2.SRC_COUNTRIES%ROWTYPE;
        L_PROCEDURE_NAME     VARCHAR2(100) := 'prc_load_SRC_2_COUNTRIES';
        ...
        SELECT
            *
        FROM
            SA_SOURCE_SYSTEM_2.SRC_COUNTRIES;
...
---------Merhe logic------------------------------------------------------------
        ...
        MERGE INTO SA_SOURCE_SYSTEM_2.src_countries CH
        USING (
                    SELECT distinct 
       country_id ,
       country_iso_code,
       ...
       SYSDATE as ta_update_dt,
       SYSDATE as ta_insert_dt
FROM SA_SOURCE_SYSTEM_2.COUNTRIES
              )
...
--------------------------------------------------------------------------------
            ROLLBACK;
            RAISE;
    END   PRC_LOAD_SRC_2_Countries;
    
    
   ----------------------------------------------------------------------
    -- Load SRC_CUSTOMERS
    ----------------------------------------------------------------------
    PROCEDURE   PRC_LOAD_SRC_2_CUSTOMERS IS

        COUNT_OF_ROWS_BEFORE NUMBER;
        COUNT_OF_ROWS_AFTER  NUMBER;
        L_ROWS               NUMBER;
        ROW_CH_CL            SA_SOURCE_SYSTEM_2.SRC_CUSTOMERS%ROWTYPE;
        L_PROCEDURE_NAME     VARCHAR2(100) := 'prc_load_SRC_2_CUSTOMERS';
        ...
        SELECT
            *
        FROM
            SA_SOURCE_SYSTEM_2.SRC_CUSTOMERS;
...
---------Merhe logic------------------------------------------------------------
        ...
        MERGE INTO SA_SOURCE_SYSTEM_2.src_customers CH
        USING (
                    SELECT distinct 
          customer_id ,
...
FROM SA_SOURCE_SYSTEM_2.customers
        ...
        WHEN MATCHED THEN UPDATE
        SET 
       customer_company_name=t.customer_company_name,
       customer_company_number=t.customer_company_number,
       ...
       customer_email= t.customer_email,
            TA_UPDATE_DT = SYSDATE
        WHERE
            ( DECODE(CH.customer_company_name, T.customer_company_name, 0, 1)
            +DECODE(CH.customer_company_number, T.customer_company_number, 0, 1)
...
--------------------------------------------------------------------------------
    ...
    -- Load SRC_EMPLOYEES
    ----------------------------------------------------------------------
    PROCEDURE   PRC_LOAD_SRC_2_EMPLOYEES IS

        COUNT_OF_ROWS_BEFORE NUMBER;
        COUNT_OF_ROWS_AFTER  NUMBER;
        L_ROWS               NUMBER;
        ROW_CH_CL            SA_SOURCE_SYSTEM_2.SRC_EMPLOYEES%ROWTYPE;
        L_PROCEDURE_NAME     VARCHAR2(100) := 'prc_load_SRC_2_EMPLOYEES';
        ...
        SELECT
            *
        FROM
            SA_SOURCE_SYSTEM_2.SRC_EMPLOYEES;
...
---------Merhe logic------------------------------------------------------------
        ...
        MERGE INTO SA_SOURCE_SYSTEM_2.src_EMPLOYEES CH
        USING (
        SELECT DISTINCT
              employee_id ,
              employee_name ,
...
--------------------------------------------------------------------------------
            ROLLBACK;
            RAISE;
    END   PRC_LOAD_SRC_2_EMPLOYEES;
    
    
     ----------------------------------------------------------------------
    -- Load SRC_PRODUCTS
    ----------------------------------------------------------------------
    PROCEDURE   PRC_LOAD_SRC_2_PRODUCTS IS

        COUNT_OF_ROWS_BEFORE NUMBER;
        COUNT_OF_ROWS_AFTER  NUMBER;
        L_ROWS               NUMBER;
        ROW_CH_CL            SA_SOURCE_SYSTEM_2.SRC_PRODUCTS%ROWTYPE;
        L_PROCEDURE_NAME     VARCHAR2(100) := 'prc_load_SRC_2_PRODUCTS';
        ...
        SELECT
            *
        FROM
            SA_SOURCE_SYSTEM_2.SRC_PRODUCTS;
...
---------Merhe logic------------------------------------------------------------
        ...
        MERGE INTO SA_SOURCE_SYSTEM_2.src_PRODUCTS CH
        USING (
        SELECT DISTINCT
              product_id  ,
             product_name,
...
--------------------------------------------------------------------------------
    ...
    -- Load SRC_PROMOTIONS
    ----------------------------------------------------------------------
    PROCEDURE   PRC_LOAD_SRC_2_PROMOTIONS IS

        COUNT_OF_ROWS_BEFORE NUMBER;
        COUNT_OF_ROWS_AFTER  NUMBER;
        L_ROWS               NUMBER;
        ROW_CH_CL            SA_SOURCE_SYSTEM_2.SRC_PROMOTIONS%ROWTYPE;
        L_PROCEDURE_NAME     VARCHAR2(100) := 'prc_load_SRC_2_PROMOTIONS';
        ...
        SELECT
            *
        FROM
            SA_SOURCE_SYSTEM_2.SRC_PROMOTIONS;
...
---------Merhe logic------------------------------------------------------------
        ...
        MERGE INTO SA_SOURCE_SYSTEM_2.src_PROMOTIONS CH
        USING (
        SELECT DISTINCT
              promo_id ,
             promo_name ,
...
FROM SA_SOURCE_SYSTEM_2.PROMOTIONS
        ...
        AND CH.PROMO_BEGIN_DATE =T.PROMO_BEGIN_DATE)
        WHEN MATCHED THEN UPDATE
        SET 
...
--------------------------------------------------------------------------------
            ROLLBACK;
            RAISE;
    END   PRC_LOAD_SRC_2_PROMOTIONS;
    
    
        ----------------------------------------------------------------------
    -- Load SRC_STORES
    ----------------------------------------------------------------------
    PROCEDURE   PRC_LOAD_SRC_2_STORES IS

        COUNT_OF_ROWS_BEFORE NUMBER;
        COUNT_OF_ROWS_AFTER  NUMBER;
        L_ROWS               NUMBER;
        ROW_CH_CL            SA_SOURCE_SYSTEM_2.SRC_STORES%ROWTYPE;
        L_PROCEDURE_NAME     VARCHAR2(100) := 'prc_load_SRC_2_STORES';
        L_TABLE              VARCHAR2(100) := 'SRC_PROMOTIONS';
        L_CONTEXT            VARCHAR2(100) := 'Merge result';
        CURSOR CH_CL IS
        SELECT
...
---------Merhe logic------------------------------------------------------------
        ...
        MERGE INTO SA_SOURCE_SYSTEM_2.src_STORES CH
        USING (
        SELECT DISTINCT
                 store_id  ,
                 store_postal_code ,
...
--------------------------------------------------------------------------------
            ROLLBACK;
            RAISE;
    END   PRC_LOAD_SRC_2_STORES;


        ----------------------------------------------------------------------
    -- Load SRC_PROMOTIONS
    ----------------------------------------------------------------------
    PROCEDURE   PRC_LOAD_SRC_2_SALES IS

        COUNT_OF_ROWS_BEFORE NUMBER;
        COUNT_OF_ROWS_AFTER  NUMBER;
        L_ROWS               NUMBER;
        ROW_CH_CL            SA_SOURCE_SYSTEM_2.SRC_SALES%ROWTYPE;
        L_PROCEDURE_NAME     VARCHAR2(100) := 'prc_load_SRC_2_SALES';
...
---------insert logic------------------------------------------------------------
 MERGE INTO SA_SOURCE_SYSTEM_2.src_sales CH
        USING (
        SELECT 
...
EXECUTE IMMEDIATE 'truncate table SA_SOURCE_SYSTEM_2.SRC_SALES';
...
--------------------------------------------------------------------------------
            ROLLBACK;
            RAISE;
    END   PRC_LOAD_SRC_2_SALES;
END PKG_LOAD_SRC_2_TABLES;